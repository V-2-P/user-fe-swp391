...
const OrderDetailPage: React.FC = () => {
  ...
  const [loading, error, response] = useFetchData(`/orders/user/detail/${id}`)
  ...
  const orderDes: DescriptionsProps['items'] = [
    ...
    },
    {
      ...
      children: formatCurrencyVND(order?.voucher?.discount)
    },
    {
      key: '4',
      label: 'Thành tiền',
    ...
    },
    {
      ...
      children: order?.paymentMethod === 'vnpay' ? 'VNPay' : 'Tiền mặt'
    }
  ]
  ...
  useEffect(() => {
    const intervalId = setInterval(async () => {
      try {
        if (order) {
          if (order.status === OrderStatus.pending && order.paymentMethod === 'vnpay') {
        ...
        {error ? (
          ...
          <Space className='w-full bg-[#fff] p-10 rounded-xl' direction='vertical' size='large'>
            ...
            <Space direction='vertical' className='!w-full px-5'>
              ...
              <Typography.Text type='secondary'>{order?.toAddress}</Typography.Text>
            </Space>
