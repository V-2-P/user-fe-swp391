import { App, Breadcrumb, Button, Image, Result, Row, Skeleton, Typography, Space, List, Card, Badge } from 'antd'
...
const ViewAllBird: React.FC = () => {
  ...
  const [loading, error, response] = useFetchData(
    `/birds/user?page=${page - 1}&limit=${limit}${keyword ? `&keyword=${keyword}` : ''}${
  ...
  const addToCompare = (bird: Bird) => {
    if (!compareItems.find((item) => item.id === bird.id.toString())) {
      if (compareItems.length > 0 && compareItems[0].detail?.birdType.name === bird.birdType.name) {
        addProductToCompare({
          id: bird.id.toString(),
          name: bird.name,
      ...
      } else if (compareItems.length > 0 && compareItems[0].detail?.birdType.name !== bird.birdType.name) {
            ...
            <Row style={{ background: '#038777' }} className='p-5 rounded-xl'>
              <List
                ...
                renderItem={(item) => (
                  <List.Item>
                    <Badge.Ribbon
                      text={item.quantity === 0 ? 'Hết hàng' : ''}
                      className='!text-lg'
                      color={item.quantity === 0 ? 'red' : 'transparent'}
                    >
                      <Card
                        ...
                        cover={
                          <Image
                            ...
                            height={300}
                            className='text-center !w-[100%] object-cover !h-full'
                            preview={false}
                      ...
                      >
                        <div className='flex flex-col text-xs space-y-3'>
                          <div className='flex flex-col justify-center w-full space-y-3'>
                            ...
                            <Typography.Text ellipsis={true}>Tồn kho: {item.quantity} </Typography.Text>
                          ...
                          <Button
                            ...
                            type='primary'
                          >
                            {item.quantity === 0 ? 'Hết hàng' : ' Thêm giỏ hàng'}
                          </Button>
                        </div>
                      </Card>
                    </Badge.Ribbon>
